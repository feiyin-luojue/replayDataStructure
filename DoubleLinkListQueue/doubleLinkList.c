...
{
    #if 0
    return DoubleLinkListGetAppointPosVal(pList, pVal, 0);
    #else
    if (!pList)
    {
        return NULL_PTR;
...
{
    #if 0
    return DoubleLinkListGetAppointPosVal(pList, &pVal, pList->len);
    #else
    if (!pList)
    {
        return NULL_PTR;
...
/* 获取链表指定位置值 */
int DoubleLinkListGetAppointPosVal(DoubleLinkList *pList, ELEMENTTYPE *pVal, int pos)
{
    if (!pList)
    {
        return NULL_PTR;
